...
def get_safe_version(repo_id: str, version: str | packaging.version.Version) -> str:
    ...
    try:
        ...
        import json, pathlib
        local_root = pathlib.Path.home() / ".cache/huggingface/lerobot" / repo_id
        info_file = local_root / "meta/info.json"
        if info_file.exists():
            with open(info_file) as f:
                info = json.load(f)
            return packaging.version.parse(info.get("codebase_version", "v3.0"))
        hub_versions = []
