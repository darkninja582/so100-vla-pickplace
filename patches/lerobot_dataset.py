...
class LeRobotDataset(torch.utils.data.Dataset):
    ...
    def _download(self, download_videos: bool = True) -> None:
        ...
        # Skip download if dataset already exists locally
        if self._requested_root is not None and self._requested_root.exists():
            info_file = self._requested_root / "meta" / "info.json"
            if info_file.exists():
